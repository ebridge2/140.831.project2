...
```{r}
## compute MGC when leveraging all accelerometers simultaneously
# iterate over each resolution
activity.agg.all.effects <- lapply(names(windowed.dat.merged), function(res) {
  res.window <- windowed.dat.merged[[res]]
  # iterate over the activities available
  activity.names <- c(unique(res.window$Activity), "Overall")
  lapply(activity.names, function(Activity) {
    dat.res.act <- res.window %>%
      select(-ID, -Activity, -window) 
    # if not all activities, compute MGC for the individual activity
    if (Activity != "Overall") {
      # compute MGC over all features at once with respect to particular activity
      p.value <- mgc.ksample(dat.res.act, res.window$Activity == Activity,
                             mgc.opts=list(nperm=100))$p.value
    } else {
      # compute MGC over all features at once with respect to particular activity
      p.value <- mgc.ksample(dat.res.act, res.window$Activity,
                             mgc.opts=list(nperm=100))$p.value
    }
    return(data.frame(Activity=Activity, Resolution=res, p.value=p.value))
  }) %>%
    bind_rows()
}) %>%
  bind_rows()

## compute MGC when leveraging each accelerometer individually
# iterate over each resolution
activity.individ.all.effects <- lapply(names(windowed.dat), function(res) {
  res.window <- windowed.dat.merged[[res]]
  # iterate over the activities available
  activity.names <- c(unique(res.window$Activity), "Overall")
  Features <- colnames(res.window)
  Features <- Features[!(Features %in% c("ID", "Activity", "Location", "window"))]
  lapply(Features, function(Feature) {
    lapply(activity.names, function(Activity) {
      ...
      if (Activity != "Overall") {
        # compute MGC over all features at once with respect to particular activity
        p.value <- mgc.ksample(feat.res, res.window$Activity == Activity)$p.value
      } else {
        # compute MGC over all features at once with respect to particular activity
        p.value <- mgc.ksample(feat.res, res.window$Activity)$p.value
      }
      return(data.frame(Activity=Activity, Resolution=res, Feature=Feature, p.value=p.value))
    }) %>%
      bind_rows()
  }) %>%
    bind_rows()
...
## compute MGC when leveraging all accelerometers simultaneously
# iterate over each resolution
activity.agg.perloc.effects <- lapply(names(windowed.dat.merged), function(res) {
  res.window <- windowed.dat.merged[[res]]
  # iterate over the activities available
  activity.names <- c(unique(res.window$Activity), "Overall")
  lapply(activity.names, function(Activity) {
    lapply(unique(res.window$Location), function(Location) {
      dat.res.act <- res.window %>%
      ...
      return(data.frame(Activity=Activity, Resolution=res, Location=Location, p.value=p.value))
    }) %>%
      bind_rows()
  }) %>%
    bind_rows()
...
## compute MGC when leveraging each accelerometer individually
# iterate over each resolution
activity.individ.perloc.effects <- lapply(names(windowed.dat), function(res) {
  res.window <- windowed.dat.merged[[res]]
  # iterate over the activities available
  activity.names <- c(unique(res.window$Activity), "Overall")
  Features <- colnames(res.window)
  ...
  lapply(Features, function(Feature) {
    lapply(activity.names, function(Activity) {
      lapply(unique(res.window$Location), function(Location) {
        ...
        return(data.frame(Activity=Activity, Resolution=res, Feature=Feature,
                          Location=Location, p.value=p.value))
      }) %>%
        bind_rows()
    }) %>%
      bind_rows()
